...
import org.ssh.jpademo.repository.search.BoardSearch;
...
//    List<Board> findByTitleAndContent(String keyword);
    ...
    @Query("select b from Board b where b.title like concat('%',:keyword, '%') or b.content like concat('%', :keyword, '%') or b.author like concat('%', :keyword, '%')")
